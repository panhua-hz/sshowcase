...
public class ConfigCtxSecurity extends WebSecurityConfigurerAdapter {
	...
	protected void securityFilterPage(HttpSecurity http, boolean isRememberMe) throws Exception{
        ...
        .formLogin()
            ...
            .permitAll()
            .and()
            //.rememberMe().tokenValiditySeconds(1440).key("mysay").and()  
            //.rememberMe().tokenRepository(new InMemoryTokenRepositoryImpl()).tokenValiditySeconds(1440).key("mynoteKey").and()
        .httpBasic()
        	.and()
        .logout()
            .permitAll();
